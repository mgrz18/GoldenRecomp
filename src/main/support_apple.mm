...
// Push an autorelease pool for the current thread.
// Using objc_autoreleasePoolPush/Pop instead of NSAutoreleasePool
// to avoid TLS destructor issues.
extern "C" void ensure_thread_autorelease_pool() {
    // Just push an autorelease pool - never pop it (thread will clean up on exit)
    @autoreleasepool {
        // Empty - the pool exists for the duration of this scope
        // But we want it to persist, so we use a different approach:
    }
    // Actually, calling objc_autoreleasePoolPush directly:
